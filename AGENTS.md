...
- **The Single Responsibility Principle applies at the class/module level, not the file level.** Grouping a Strategy-pattern ABC, its concrete implementations, and its factory function in one file is normal and encouraged here, as long as they share one cohesive concept and one reason to change (e.g. `drivers/embedding.py`, `drivers/llm.py`, and `migrations/base.py` alongside the migration files it supports). Splitting these into one-class-per-file would not improve SRP — it would just scatter one responsibility across more files.
