...
Run `uv run python scripts/inspect_chunks.py` (uses `TEST_DOC_PATH` by default, or pass a path — PDF or Markdown) to see all of this at once for your own documents: one table, every `PDF_EXTRACTION_MODE` x `CHUNKING_STRATEGY` x `CHUNK_OVERFLOW_STRATEGY` combination as its own row, each with a bar showing that row's *worst* chunk against the model's real token limit — so which combination actually needs the least correction is a glance, not six separate reports to compare by hand.
